...
pub struct AnalysisResult {
    pub bpm: f32,
    pub is_drop: bool,
    pub beat_offset: Option<Duration>, // Temps écoulé depuis le beat (latency)
    pub confidence: f32,
...
impl BpmAnalyzer {
    ...
    pub fn process(&mut self, new_samples: &[f32]) -> AnalysisResult {
        ...
        //Calcul précis du moment du Drop (Offset temporel)
        let mut beat_offset = None;
        if is_drop {
            // On cherche le pic d'énergie dans les derniers 25% du buffer (là où check_drop regarde)
            let split_index = (self.scratch_fine_vec.len() * 3) / 4;
            if let Some((idx, _)) = self.scratch_fine_vec[split_index..]
                .iter()
                .enumerate()
                .max_by(|a, b| a.1.partial_cmp(b.1).unwrap_or(std::cmp::Ordering::Equal))
            {
                let absolute_idx = split_index + idx;
                // Combien d'échantillons se sont écoulés depuis ce pic jusqu'à la fin du buffer ?
                let samples_since_peak =
                    self.scratch_fine_vec.len().saturating_sub(1 + absolute_idx);
                let seconds_since_peak = samples_since_peak as f32 / self.fine_config.rate;
                beat_offset = Some(Duration::from_secs_f32(seconds_since_peak));
            }
        }
